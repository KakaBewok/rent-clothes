                            ...
                            <Link href="#">
                                {product.additional_ribbon && (
                                    <div className="absolute top-0 left-0 z-10 rounded-tl-sm rounded-br-sm bg-first px-2 py-1">
                                        <p className="text-xs font-normal text-white">{product.additional_ribbon}</p>
                                    </div>
                                )}
                                <img
                                    alt={product.name}
                                ...
                                <div className="mt-2 flex justify-between px-0 md:px-1">
                                    <div className="space-y-1 lg:space-y-1.5">
                                        ...
                                        <h3 className="text-xs text-slate-500">By {product.brand?.name}</h3>
                                        <p className="text-xs text-slate-500">
                                            {new Intl.NumberFormat('id-ID', {
                                                style: 'currency',
                                                currency: 'IDR',
                                                minimumFractionDigits: 0,
                                            }).format(product.price_detail?.price_after_discount ?? 0)}
                                            /1-{product.rent_periode} days
                                        </p>
                                        {product.types && product.types.length > 0 && (
                                            <div className="flex items-center gap-1">
                                                <Tag className="h-5 w-5 text-green-500" />
                                                <p className="text-xs font-semibold text-green-500">{product.types[0].name}</p>
                                            </div>
                                        )}
                                    </div>
                                    <p className="text-xs font-medium text-slate-600">{product.sizes?.[0].size}</p>
                                </div>
                            </Link>
                        </div>
                    ))}
